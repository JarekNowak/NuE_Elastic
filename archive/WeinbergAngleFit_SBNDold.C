...
namespace Setup {
  ...
  const Double_t N_ELECTRONS =  2.437e31; // e⁻ targets in fiducial vol. // (ρ≈1.38 g/cm³, 89.82 t fiducial) 
...
/// Maximum recoil energy kinematically allowed.
...
TH1D* BuildTrueSpectrum(Double_t sin2thW) {
  // Use same binning as efficiency histogram (true energy)
  ...
  for (Int_t iT = 1; iT <= nT; ++iT) {
    ...
    for (Int_t iE = 1; iE <= nEnu; ++iE) {
      ...
      // Only contribute if T_c is kinematically accessible
      if (T_c > T_max(Enu)) continue;

      // Integrate cross-section over the T bin (trapezoidal, 4 sub-steps)
      Int_t nSub = 4;
      Double_t integral = 0.0;
      for (Int_t k = 0; k < nSub; ++k) {
        Double_t T1 = T_lo + (T_hi - T_lo) * k       / nSub;
        Double_t T2 = T_lo + (T_hi - T_lo) * (k + 1) / nSub;
        integral += 0.5 * (dsigma_dT(T1, Enu, sin2thW)
                         + dsigma_dT(T2, Enu, sin2thW))
                    * (T2 - T1);
      }

      rate += flux * dEnu * integral; // [cm²/POT/GeV] * GeV * [cm²/GeV] -- but flux already /GeV
      // Units: flux [/cm²/POT/GeV] * dEnu [GeV] * xsec [cm²/GeV] * dT [GeV]
      //        => [/POT] × cm² × cm² — need to be careful:
      // Actually: dΦ/dEν × dEν  →  [/cm²/POT]
      //           dσ/dT  × dT   →  [cm²]
      // rate [/POT/target] += flux×dEnu × xsec×dT
    }

    // Multiply by number of target electrons and POT
...
void FCN(Int_t& npar, Double_t* grad, Double_t& fval,
  ...
  for (Int_t i = 1; i <= nBins; ++i) {
    ...
    // Use statistical uncertainty; for empty predicted bins use Poisson
    if (err <= 0) err = 1.0;
...
// Replace this section entirely when running with real SBND input files.
...
void GeneratePlaceholderHistograms() {
  ...
  g_hFlux = new TH1D("h_flux", "BNB #nu_{#mu} flux;E_{#nu} [GeV];#Phi [cm^{-2}POT^{-1}GeV^{-1}]", nE, Elo, Ehi);
  for (Int_t i = 1; i <= nE; ++i) {
    Double_t E   = g_hFlux->GetBinCenter(i);
    Double_t phi = 1.2e-11 * TMath::Gaus(E, 0.7, 0.25); // peak at ~700 MeV
    g_hFlux->SetBinContent(i, phi);
  }

  // ── Efficiency (flat-ish, falling at high T) ──────────────────────────────
  g_hEff = new TH1D("h_eff", "Efficiency;T_{e} [GeV];#epsilon",
                    nT, Tlo, Thi);
  ...
  // ── Smearing matrix (Gaussian blur σ ≈ 5% of true value) ─────────────────
  ...
  for (Int_t iT = 1; iT <= nT; ++iT) {
    ...
    Double_t sigmaT = 0.05 * Ttrue + 0.01;  // energy resolution
    Double_t colSum = 0.0;
    ...
    // Normalise column so probabilities sum to 1
    if (colSum > 0) {
      for (Int_t iR = 1; iR <= nT; ++iR)
  ...
  // ── Signal prediction at SM value (to create fake "data") ────────────────
...
void DrawResult(Double_t sin2thW_fit, Double_t sin2thW_err,
  ...
  hUp->SetLineColor(kRed - 7);
  hUp->SetLineStyle(2);
  hDown->SetLineColor(kRed - 7);
  hDown->SetLineStyle(2);

  TH1D* hBkg = (TH1D*)g_hBkg->Clone("h_bkg_plot");
  ...
  delete hBestFit; delete hUp; delete hDown; delete hBkg;
}
...
void WeinbergAngleFit_SBND(const char* inputFile = "") {
  ...
  if (inputFile && strlen(inputFile) > 0) {
    ...
    if (!fIn || fIn->IsZombie()) {
      ...
      if (!g_hData || !g_hEff || !g_hSmear || !g_hFlux) {
        ...
        // Detach from file so they survive
  ...
  if (usePlaceholder) {
    // Need flux to be initialised before GeneratePlaceholderHistograms
    // calls BuildTrueSpectrum, so set a temporary flux first
    const Int_t nE = 40;
    g_hFlux = new TH1D("h_flux_tmp","",nE,0,3);
    // Temporary efficiency too
    g_hEff  = new TH1D("h_eff_tmp","",20,0,1);
    for(Int_t i=1;i<=20;++i) g_hEff->SetBinContent(i,0.7);
    // Temp smear (identity)
    g_hSmear = new TH2D("h_smear_tmp","",20,0,1,20,0,1);
    for(Int_t i=1;i<=20;++i) g_hSmear->SetBinContent(i,i,1.0);
    g_hBkg  = nullptr;
  ...
  minuit.SetErrorDef(1.0); // Δχ² = 1 for 1σ

  Double_t arglist[10];
  Int_t    ierflg = 0;

  // Define parameter: sin²θ_W
  ...
  // Improve with HESSE (proper uncertainty)
  arglist[0] = 2000;
  ...
  if (fOut && !fOut->IsZombie()) {
    ...
    results.Write("fit_results"); // [sin2thW, err, chi2, ndf]
